...
Handler = {
    ...
    MatchDetails: function () {
        ...
        if ( Data.predictable == false ) {
            $('[data-dom=predict-tab-link]').remove();
            $('[data-dom=predict-tab-block]').remove();
        }
    ...
    MatchInfo: function () {
        ...
        Request.getPredictionList ( {
            ...
            callback: res => {
                console.log ( res );
                const predictForm = $ ( '[data-dom=predict-form]' );
                const predictContent = $ ( '[data-dom=predict-container]' );
                const gamePredictions   =   res.gamePredictions;
                const calcualtion = gamePredictions.calculation ? gamePredictions.calculation : 0;

                if ( !calcualtion.length ) {
                    predictContent.find ( '[data-dom=head]' ).addClass ( 'waiting' );
                    predictContent.find ( '[data-dom=headTitle]' ).text ( Language[ Help.LangName () ][ 'txtWaiting' ] );
                    predictContent.find ( '[data-dom=status]' ).text ( Language[ Help.LangName () ][ 'txtWaiting' ] );
                }
                else if ( calcualtion.length ) {
                 ...
                 * 0 -> f
                 * Match Score -> V
                 * Her hansi biri ferqli 0 -> success
                 * */

                if ( res.gamePredictions.length > 0 ) {
                    predictForm.remove ();
                    predictContent.removeClass ( 'display-none' );
                    predictContent.find ( '[data-dom=home]' ).html ( res.gamePredictions[ 0 ].predictionValue.split ( '-' )[ 0 ] );
                    predictContent.find ( '[data-dom=away]' ).html ( res.gamePredictions[ 0 ].predictionValue.split ( '-' )[ 1 ] );
                    Request.getPredictionBonus ( {
                        value: {
                            token: Help.Token (),
                            matchId: Help.Url ( 3 ),
                            predictionValue: res.gamePredictions[ 0 ].predictionValue
                        },
                        callback: response => {
                            console.log ( response );
                            const noteLanguage = Language[ Help.LangName () ][ 'txtPredictAddNotficationText' ];
                            $ ( '[data-dom=bonus]' ).find ( '[data-dom=coin]' ).text ( response.bonus );
                            $ ( '[data-dom=result]' ).find ( '[data-dom=coin]' ).text ( response.result );
                            $ ( '[data-dom=match-score]' ).find ( '[data-dom=coin]' ).text ( response.matchScore );
                            $ ( '[data-dom=total-goals]' ).find ( '[data-dom=coin]' ).text ( response.totalGoals );
                            $ ( '[data-dom=goals-away]' ).find ( '[data-dom=coin]' ).text ( response.goalsScoredByAway );
                            $ ( '[data-dom=goals-home]' ).find ( '[data-dom=coin]' ).text ( response.goalsScoredByHome );
                            $ ( '[data-lang=txtPredictAddNotficationText]' ).html ( noteLanguage.replace ( '{bonus}', response.bonus ).replace ( '{point}', response.point ) );
                        }
                    } )
                }
                else {
                    predictContent.remove ();
                    predictForm.removeClass ( 'display-none' );
                }
            }
        } );
    ...
    PredictionList: params => {
        ...
        Request.getPredictionList ( {
            ...
            callback: res => {
                ...
                const lists = res.gamePredictions;
                const map = {
                    content: $ ( '[data-dom=predict-wrapper]' ),
                    block: $ ( '[data-dom=predict-list]' ).clone ().removeClass ( 'display-none' ).removeAttr ( 'data-dom' )
                };

                if ( lists.length ) {

                    $.each ( lists, ( key, val ) => {
                        map.finalBlock = map.block.clone ();

                        /* Prediction Data */
                        const predictData = {
                            score: val.match.score,
                            teamHome: val.match.team1Name,
                            teamHomePhoto: ApiUrl + ApiPath.Match.teamLogo.replace ( '{teamId}', val.match.team1Id ),
                            teamAway: val.match.team2Name,
                            teamAwayPhoto: ApiUrl + ApiPath.Match.teamLogo.replace ( '{teamId}', val.match.team2Id ),
                            matchId: val.matchId,
                            predictValue: val.predictionValue,
                            matchStatus: Help.MatchStatus ( {
                                'Type': 'status',
                        ...
                        Help.dom ( {
                            parent: map.finalBlock,
                            tags: [
                                /* Home Photo */
                                {
                                    name: '[data-dom=homePhoto]',
                                    child: 'img',
                                    attr: { 'src': predictData.teamHomePhoto }
                                },
                                /* Home Name */
                                {
                                    name: '[data-dom=homeName]',
                                    text: predictData.teamHome
                                },
                                /* Home Prediction */
                                {
                                    name: '[data-dom=homePredict]',
                                    text: predictData.predictValue.split ( '-' )[ 0 ]
                                },
                                /* Away Photo */
                                {
                                    name: '[data-dom=awayPhoto]',
                                    child: 'img',
                                    attr: { 'src': predictData.teamAwayPhoto }
                                },
                                /* Away Name */
                                {
                                    name: '[data-dom=awayName]',
                                    text: predictData.teamAway
                                },
                                /* Away Prediction */
                                {
                                    name: '[data-dom=awayPredict]',
                                    text: predictData.predictValue.split ( '-' )[ 1 ]
                                },
                                /* Status And Score */
                                {
                                    name: '[data-dom=score]',
                                    text: predictData.matchStatus
                                }
                            ],
                            callback: () => {
                                map.content.append ( map.finalBlock );
                            }
                        } );
